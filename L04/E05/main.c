...
void selezionaDati(comando_e cmd, int n_tratte, Tratta* ordinamenti[N_ORDINAMENTI][n_tratte], int *tratte_ordinate_partenza, comando_e* ultimo_ordinamento);
void stampa(int n_tratte, Tratta* tratte[n_tratte]);
void ordina(int n_tratte, Tratta* tratte[n_tratte], comando_e cmd);
int confronta(Tratta t1, Tratta t2, comando_e cmd);
void cerca(int n_tratte, Tratta* tratte[n_tratte], int tratte_ordinate_partenza);
void ricercaDicotomica(char cerca[], int n_tratte, Tratta* tratte[n_tratte]);
...
int ora_toint(Ora o);
void riempiTratte(FILE* fp, int n_tratte, Tratta tratte[], Tratta* ordinamenti[N_ORDINAMENTI][n_tratte]);
...
int main() {
    ...
    Tratta* ordinamenti[N_ORDINAMENTI][n_tratte];
    riempiTratte(fp, n_tratte, tratte, ordinamenti);

    comando_e cmd;
    comando_e ultimo_ordinamento;
    ...
    do {
        cmd = leggiComando();
        selezionaDati(cmd, n_tratte, ordinamenti, &tratte_ordinate_partenza, &ultimo_ordinamento);
    } while(cmd != r_fine);
...
comando_e leggiComando() {
    ...
    char* tabella[r_sconosciuto] = {"ordina_data", "ordina_cod", "ordina_partenza", "ordina_arrivo", "stampa", "cerca", "fine"};
